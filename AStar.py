...
class AStar(object):
    ...
    def __init__(self):
        ...
        self.open = numpy.array([[], [], [], [], [], []])  # 先创建一个空的open表, 记录坐标x,坐标y，方向x,方向y，g值，f值
    ...
    def child_point(self, x):
        ...
        for j in range(-1, 2, 1):
            for q in range(-1, 2, 1):
                ...
                record_g = self.g_accumulation(m, x)  # 计算g值
                ...
                if b == 1:
                    # 说明closed中已经存在这个点，线从close表中删除再添加回open表中去
                    ...
                    if record_f <= self.closed[5][index2]:
                        ...
                        self.closed = numpy.delete(self.closed, index2, axis=1)  # 参数从closed中去除
                        # numpy.c_按列连接两个矩阵
                        self.open = numpy.c_[self.open, para]  # 参数添加到open中
                    continue
                    # 不在closed表中直接添加在open表中
                self.open = numpy.c_[self.open, para]  # 参数添加到open中
                print(self.open)
    ...
    def main(self):
        ...
        while ite <= 1000:
                ...
                # 对closed的更新在这里，而不是上面的child_point
                best = self.open[:, 0]
